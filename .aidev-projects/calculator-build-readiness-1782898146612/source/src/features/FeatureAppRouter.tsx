...
export default function FeatureAppRouter() {
  const [activeModuleId, setActiveModuleId] = useState('calculator');
  ...
  return (
    ...
    >
      ...
      <nav className="modular-nav" aria-label="Feature modules">
        <button
          type="button"
          className={`modular-nav-item ${activeModuleId === 'calculator' ? 'is-active' : ''}`}
          onClick={() => setActiveModuleId('calculator')}
        >
          Calculator
        </button>
      </nav>
