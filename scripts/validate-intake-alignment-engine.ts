...
function buildRideSharingFalseConflictFixture(): {
  ...
  const conflicts: EvidenceConflict[] = [
    {
      readOnly: true,
      conflictId: 'conflict-platform-1',
      ...
      resolutionHint: 'Clarify primary platform',
    },
    {
      readOnly: true,
      conflictId: 'conflict-role-1',
      ...
      resolutionHint: 'Clarify user roles',
    },
  ];
  ...
  const intake: UnifiedIntakeAnalysis = {
    ...
    intakeReadiness: 'NEEDS_CLARIFICATION',
    intakeRecommendations: [],
  };

  const voice: VoiceNotesAnalysis = {
    readOnly: true,
    analysisId: 'voice-ride-fixture',
    analyzedAt: new Date().toISOString(),
    transcript: {
      readOnly: true,
      text: 'Drivers and riders connect through a mobile app with ride requests, tracking, and payments.',
      confidence: 88,
      language: 'en',
      wordCount: 14,
    },
    ...
    projectUnderstanding: {
      ...
      productType: 'MOBILE_APP',
      platformTargets: ['iOS', 'Android'],
      primaryPurpose: 'Transportation marketplace',
      confidence: 85,
    },
    voiceNotesConfidence: 85,
    evidence: ['VOICE_TRANSCRIPT', 'REQUIREMENTS_EXTRACTED'],
  };

  const visual: VisualReferenceAnalysis = {
    readOnly: true,
    ...
    screenDetection: {
      readOnly: true,
      platform: 'MOBILE',
      screenType: 'TRANSPORTATION_UI',
      layoutPattern: 'BOTTOM_NAVIGATION',
      confidence: 82,
    },
    uiComponents: {
      readOnly: true,
      detectedComponents: ['BOTTOM_NAVIGATION', 'MAP_VIEW', 'CTA_BUTTON'],
      navigationPattern: 'BOTTOM_TABS',
      colorPalette: ['#1c202c', '#ebeff5'],
    },
    inferredFlows: ['RIDE_REQUEST', 'TRACKING'],
    visualReferenceConfidence: 80,
    evidence: ['MOBILE_UI_DETECTED', 'TRANSPORTATION_PATTERN'],
  };
...
const realAlignment = assessIntakeAlignment({
  ...
  voiceNotesAnalysis: {
    ...
    analyzedAt: new Date().toISOString(),
    transcript: { readOnly: true, text: 'Mobile-only iOS and Android app', confidence: 80, language: 'en', wordCount: 5 },
    requirements: {
      readOnly: true,
      screens: ['dashboard'],
      userRoles: ['user'],
    ...
    projectUnderstanding: {
      ...
      productType: 'MOBILE_APP',
      platformTargets: ['iOS', 'Android'],
      primaryPurpose: 'Mobile app',
      confidence: 80,
    },
    voiceNotesConfidence: 80,
