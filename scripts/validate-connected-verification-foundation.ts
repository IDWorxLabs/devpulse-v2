/**
 * Phase 25.23 — Connected Verification Foundation validation (leaf mode).
 */
...
import {
  ...
  buildConnectedVerificationReportMarkdown,
...
const REQUIRED_FILES = [
  'src/connected-verification-foundation/connected-verification-types.ts',
  'src/connected-verification-foundation/connected-verification-registry.ts',
  'src/connected-verification-foundation/connected-verification-authority.ts',
  'src/connected-verification-foundation/connected-verification-history.ts',
  'src/connected-verification-foundation/connected-verification-report-builder.ts',
  'src/connected-verification-foundation/index.ts',
  'architecture/CONNECTED_VERIFICATION_FOUNDATION_REPORT.md',
] as const;
...
function main(): void {
  console.log('');
  console.log('Connected Verification Foundation — Validation (leaf mode)');
  console.log('===========================================================');
  console.log('');
  ...
  const authoritySource = readText('src/connected-verification-foundation/connected-verification-authority.ts');
  const reportBuilderSource = readText(
    'src/connected-verification-foundation/connected-verification-report-builder.ts',
  );
  const reportMd = readText('architecture/CONNECTED_VERIFICATION_FOUNDATION_REPORT.md');
  const pkg = JSON.parse(readText('package.json')) as { scripts?: Record<string, string> };

  assert(
    'package script registered',
    Boolean(pkg.scripts?.['validate:connected-verification-foundation']),
    'package.json',
  );

  assert(
    'core question registered',
    CONNECTED_VERIFICATION_CORE_QUESTION.includes('capable of being verified'),
    CONNECTED_VERIFICATION_CORE_QUESTION,
  );

  assert(
    'orchestration flow documented',
    ORCHESTRATION_FLOW.includes('Verification Readiness Contract') &&
      ORCHESTRATION_FLOW.includes('Verification Readiness Assessment'),
  ...
  assert(
    ...
    VERIFICATION_STATES.length === 5 &&
      VERIFICATION_STATES.includes('VERIFICATION_READY') &&
      VERIFICATION_STATES.includes('INSUFFICIENT_EVIDENCE'),
    VERIFICATION_STATES.join(', '),
  );

  assert(
    'consumes live preview foundation',
    authoritySource.includes('assessConnectedLivePreview'),
    'assessConnectedLivePreview',
  );

  assert(
    'consumes runtime activation foundation via snapshot',
  ...
  assert(
    ...
    authoritySource.includes('assessVerificationReality') &&
      authoritySource.includes('detectVerificationModulePresenceEvidence'),
  ...
  assert(
    'verification readiness contract builder exported',
    authoritySource.includes('export function buildVerificationReadinessContract'),
    'buildVerificationReadinessContract',
  );

  assert(
    'no real verification execution guarantee',
    VERIFICATION_READINESS_SAFETY_GUARANTEES.some((g) => /realVerificationExecutionPerformed always false/i.test(g)),
    'execution ban',
  );
  ...
  assert(
    ...
    reportBuilderSource.includes('Verification Readiness Score') &&
      reportBuilderSource.includes('Missing Verification Components') &&
      reportBuilderSource.includes('Verification Path') &&
      reportBuilderSource.includes('Coverage Completeness'),
    'report fields',
  );

  assert(
    'architecture report pass token',
    reportMd.includes(CONNECTED_VERIFICATION_FOUNDATION_PASS_TOKEN),
    CONNECTED_VERIFICATION_FOUNDATION_PASS_TOKEN,
  );
  ...
  assert(
    'verification ready scenario',
    ready.report.verificationState === 'VERIFICATION_READY' &&
      ready.orchestrationState === 'VERIFICATION_READINESS_COMPLETE',
    ready.report.verificationState,
  );

  assert(
    'verification readiness contract generated (ready)',
    ready.report.verificationReadinessContract.contractId.length > 0 &&
      ready.report.verificationReadinessContract.realVerificationExecutionPerformed === false,
    ready.report.verificationReadinessContract.contractId,
  );

  assert(
    'contract arrays populated (ready)',
    ready.report.verificationReadinessContract.verificationSteps.length > 0 &&
      ready.report.verificationReadinessContract.verificationCoverage.length > 0 &&
      ready.report.verificationReadinessContract.proofArtifacts.length > 0,
    `steps=${ready.report.verificationReadinessContract.verificationSteps.length}`,
  );

  assert(
    'verification candidate generated (ready)',
    ready.report.verificationCandidate.candidateId.length > 0 &&
      ready.report.verificationCandidate.realVerificationExecutionPerformed === false,
    ready.report.verificationCandidate.candidateId,
  );

  assert(
    'ten required questions answered (ready)',
    Object.keys(ready.report.questionAnswers).length === 10 &&
      ready.report.questionAnswers.verificationReadinessProven === true,
    `${Object.keys(ready.report.questionAnswers).length} answers`,
  );

  assert(
    'founder report fields present (ready)',
    ready.report.verificationReadinessScore >= 80 &&
      ready.report.verificationPath.length >= 3 &&
      ready.report.verificationCompleteness >= 0 &&
      ready.report.coverageCompleteness >= 0 &&
      ready.report.proofCompleteness > 0 &&
      ready.report.recommendedNextActions.length > 0,
    `score=${ready.report.verificationReadinessScore}`,
  );
  ...
  assert(
    'verification ready with warnings scenario',
    warnings.report.verificationState === 'VERIFICATION_READY_WITH_WARNINGS',
  ...
  assert(
    'verification blocked scenario',
    blocked.report.verificationState === 'VERIFICATION_BLOCKED',
  ...
  assert(
    ...
    getConnectedVerificationHistorySize() >= 4,
    `${getConnectedVerificationHistorySize()} entries`,
  );

  const historySummary = buildConnectedVerificationHistorySummary();
  assert(
    'history summary tracks verification states',
    historySummary.totalAssessments >= 4 &&
      historySummary.readyVerifications >= 1 &&
      historySummary.readyWithWarningsVerifications >= 1 &&
      historySummary.blockedVerifications >= 1,
  ...
  const artifacts = buildConnectedVerificationArtifacts({
    connectedLivePreviewAssessment: previewAssessment,
    founderTestAssessment: founderTest,
  });
  const markdown = buildConnectedVerificationReportMarkdown(artifacts.connectedVerificationAssessment.report);
  assert(
    'artifacts bundle includes markdown report',
    artifacts.connectedVerificationReportMarkdown.length > 100 &&
      markdown.includes('Verification State') &&
      markdown.includes('Verification Readiness Contract'),
    `${artifacts.connectedVerificationReportMarkdown.length} chars`,
  );
  checkpoint('fixture scenarios');

  resetConnectedVerificationModuleForTests();
  const live = assessConnectedVerification({ rootDir: ROOT });
  checkpoint('live assessment');
  assert(
    'live assessment executes',
    live.report.verificationConnectionId.length > 0 &&
      live.report.verificationState.length > 0 &&
      live.report.verificationReadinessContract.realVerificationExecutionPerformed === false,
    `${live.report.verificationState} id=${live.report.verificationConnectionId}`,
  );
  ...
  if (failed > 0) {
    console.log('CONNECTED_VERIFICATION_FOUNDATION_REQUIRES_FIXES');
    process.exit(1);
  }

  console.log(CONNECTED_VERIFICATION_FOUNDATION_PASS_TOKEN);
  console.log('');
  console.log('Verification states verified:');
  console.log(`  VERIFICATION_READY:               ${ready.report.verificationState}`);
  console.log(`  VERIFICATION_READY_WITH_WARNINGS: ${warnings.report.verificationState}`);
  console.log(`  VERIFICATION_BLOCKED:             ${blocked.report.verificationState}`);
  ...
  console.log('');
  console.log('Report: architecture/CONNECTED_VERIFICATION_FOUNDATION_REPORT.md');
  console.log('');
}
