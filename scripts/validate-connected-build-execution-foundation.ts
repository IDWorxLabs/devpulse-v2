/**
 * Phase 25.20 — Connected Autonomous Build Execution Foundation validation (leaf mode).
 */
...
const REQUIRED_FILES = [
  'src/connected-build-execution-foundation/connected-build-execution-types.ts',
  'src/connected-build-execution-foundation/connected-build-execution-registry.ts',
  'src/connected-build-execution-foundation/connected-build-execution-authority.ts',
  'src/connected-build-execution-foundation/connected-build-execution-history.ts',
  'src/connected-build-execution-foundation/connected-build-execution-report-builder.ts',
  'src/connected-build-execution-foundation/index.ts',
  'architecture/CONNECTED_BUILD_EXECUTION_FOUNDATION_REPORT.md',
] as const;
...
function main(): void {
  console.log('');
  console.log('Connected Autonomous Build Execution Foundation — Validation (leaf mode)');
  console.log('=========================================================================');
  console.log('');
  ...
  const authoritySource = readText('src/connected-build-execution-foundation/connected-build-execution-authority.ts');
  const reportBuilderSource = readText(
    'src/connected-build-execution-foundation/connected-build-execution-report-builder.ts',
  );
  const reportMd = readText('architecture/CONNECTED_BUILD_EXECUTION_FOUNDATION_REPORT.md');
  const pkg = JSON.parse(readText('package.json')) as { scripts?: Record<string, string> };

  assert(
    'package script registered',
    Boolean(pkg.scripts?.['validate:connected-build-execution-foundation']),
    'package.json',
  );

  assert(
    'core question registered',
    CONNECTED_BUILD_EXECUTION_CORE_QUESTION.includes('verifiable build output'),
    CONNECTED_BUILD_EXECUTION_CORE_QUESTION,
  );

  assert(
    'orchestration flow documented',
    ORCHESTRATION_FLOW.includes('Build Output Manifest') && ORCHESTRATION_FLOW.includes('BUILD_OUTPUT_COMPLETE'),
  ...
  assert(
    ...
    REQUIRED_INPUT_AUTHORITIES.length === 9 &&
      REQUIRED_INPUT_AUTHORITIES.includes('autonomous-builder-execution-planner') &&
      REQUIRED_INPUT_AUTHORITIES.includes('world2-dry-run-execution-composer'),
    REQUIRED_INPUT_AUTHORITIES.join(', '),
  );

  assert(
    'build output states registered',
    BUILD_OUTPUT_STATES.length === 5 &&
      BUILD_OUTPUT_STATES.includes('BUILD_OUTPUT_PROVEN') &&
      BUILD_OUTPUT_STATES.includes('INSUFFICIENT_EVIDENCE'),
    BUILD_OUTPUT_STATES.join(', '),
  );

  assert(
    'consumes execution planner',
    authoritySource.includes('assessAutonomousBuilderExecutionPlanner'),
    'assessAutonomousBuilderExecutionPlanner',
  );

  assert(
    'consumes repair loop',
    authoritySource.includes('repairLoopAssessment'),
    'repairLoopAssessment',
  );

  assert(
    'consumes dry-run composer',
  ...
  assert(
    'consumes change set via snapshot',
    authoritySource.includes('changeSetAssessment'),
    'changeSetAssessment',
  );

  assert(
    'consumes workspace population via snapshot',
    authoritySource.includes('workspacePopulationAssessment'),
    'workspacePopulationAssessment',
  );

  assert(
    'consumes workspace materialization via snapshot',
    authoritySource.includes('workspaceMaterializationAssessment'),
    'workspaceMaterializationAssessment',
  );

  assert(
    'consumes repository snapshot via snapshot',
    authoritySource.includes('repositorySnapshotAssessment'),
    'repositorySnapshotAssessment',
  );

  assert(
    'consumes change set materializer via snapshot',
    authoritySource.includes('changeSetMaterializerAssessment'),
    'changeSetMaterializerAssessment',
  );

  assert(
    'build output manifest builder exported',
    authoritySource.includes('export function buildBuildOutputManifest'),
    'buildBuildOutputManifest',
  );

  assert(
    'no real file mutation guarantee',
    BUILD_OUTPUT_SAFETY_GUARANTEES.some((g) => /realFileMutationPerformed always false/i.test(g)),
    'mutation ban',
  );
  ...
  assert(
    ...
    reportBuilderSource.includes('Build Output Score') &&
      reportBuilderSource.includes('Missing Build Components') &&
  ...
  assert(
    ...
    reportMd.includes(CONNECTED_BUILD_EXECUTION_FOUNDATION_PASS_TOKEN),
    CONNECTED_BUILD_EXECUTION_FOUNDATION_PASS_TOKEN,
  );
  checkpoint('static checks');

  resetConnectedBuildExecutionModuleForTests();
  const provenDryRun = buildDryRunComposerAssessment(
    buildPlan('LOW'),
    buildExecutionProof('PROVEN_FIXED'),
    buildAcceptance('ACCEPTED'),
    buildFounderTest('FOUNDER_READY', true, 92),
  );
  const proven = assessConnectedAutonomousBuildExecution({
    dryRunComposerAssessment: provenDryRun,
  });
  assert(
    'proven build output scenario',
    proven.report.buildOutputState === 'BUILD_OUTPUT_PROVEN' && proven.orchestrationState === 'BUILD_OUTPUT_COMPLETE',
    proven.report.buildOutputState,
  );

  assert(
    'build manifest generated (proven)',
    proven.report.buildOutputManifest.manifestId.length > 0 &&
      proven.report.buildOutputManifest.realFileMutationPerformed === false,
    proven.report.buildOutputManifest.manifestId,
  );

  assert(
    'manifest arrays populated (proven)',
    proven.report.buildOutputManifest.expectedArtifacts.length > 0 &&
      proven.report.buildOutputManifest.proofArtifacts.length > 0 &&
      proven.report.buildOutputManifest.verificationArtifacts.length > 0 &&
      proven.report.buildOutputManifest.rollbackArtifacts.length > 0,
    `artifacts=${proven.report.buildOutputManifest.expectedArtifacts.length}`,
  );

  assert(
    'ten required questions answered (proven)',
    Object.keys(proven.report.questionAnswers).length === 10 &&
      proven.report.questionAnswers.buildOutputProven === true,
    `${Object.keys(proven.report.questionAnswers).length} answers`,
  );

  assert(
    'founder report fields present (proven)',
    proven.report.buildOutputScore >= 80 &&
      proven.report.expectedGeneratedFiles.length >= 0 &&
      proven.report.expectedGeneratedArtifacts.length > 0 &&
      proven.report.outputCompleteness >= 0 &&
      proven.report.proofCompleteness > 0 &&
      proven.report.recommendedNextActions.length > 0,
    `score=${proven.report.buildOutputScore}`,
  );

  const partialDryRun = buildDryRunComposerAssessment(
    buildPlan('HIGH'),
  ...
  const partial = assessConnectedAutonomousBuildExecution({
    dryRunComposerAssessment: partialDryRun,
  });
  assert(
    'partially proven build output scenario',
    partial.report.buildOutputState === 'BUILD_OUTPUT_PARTIALLY_PROVEN',
    partial.report.buildOutputState,
  );

  const blockedDryRun = buildDryRunComposerAssessment(
    buildPlan('CRITICAL'),
    buildExecutionProof('PROVEN_FIXED'),
    buildAcceptance('BLOCKED'),
    buildFounderTest('BLOCKED', true, 40),
  );
  const blocked = assessConnectedAutonomousBuildExecution({
    dryRunComposerAssessment: blockedDryRun,
  });
  assert(
    'blocked build output scenario',
    blocked.report.buildOutputState === 'BUILD_OUTPUT_BLOCKED',
    blocked.report.buildOutputState,
  );

  const insufficientDryRun = buildDryRunComposerAssessment(
    buildPlan('MEDIUM'),
    null,
    null,
    buildFounderTest('INSUFFICIENT_EVIDENCE', false, 30),
  );
  const insufficient = assessConnectedAutonomousBuildExecution({
    dryRunComposerAssessment: insufficientDryRun,
  });
  assert(
    'insufficient evidence scenario',
    insufficient.report.buildOutputState === 'INSUFFICIENT_EVIDENCE',
    insufficient.report.buildOutputState,
  );

  assert(
    'history records assessments',
    getConnectedBuildExecutionHistorySize() >= 4,
    `${getConnectedBuildExecutionHistorySize()} entries`,
  );

  const historySummary = buildConnectedBuildExecutionHistorySummary();
  assert(
    'history summary tracks proof states',
    historySummary.totalAssessments >= 4 &&
      historySummary.provenOutputs >= 1 &&
      historySummary.partiallyProvenOutputs >= 1 &&
      historySummary.blockedOutputs >= 1,
    JSON.stringify(historySummary),
  );

  resetConnectedBuildExecutionModuleForTests();
  const artifacts = buildConnectedBuildExecutionArtifacts({
    dryRunComposerAssessment: provenDryRun,
  });
  const markdown = buildConnectedBuildExecutionReportMarkdown(artifacts.connectedBuildExecutionAssessment.report);
  assert(
    'artifacts bundle includes markdown report',
    artifacts.connectedBuildExecutionReportMarkdown.length > 100 &&
      markdown.includes('Build Output State') &&
      markdown.includes('Build Output Manifest'),
    `${artifacts.connectedBuildExecutionReportMarkdown.length} chars`,
  );
  checkpoint('fixture scenarios');

  resetConnectedBuildExecutionModuleForTests();
  const live = assessConnectedAutonomousBuildExecution({ rootDir: ROOT });
  checkpoint('live assessment');
  assert(
    'live assessment executes',
    live.report.connectionId.length > 0 &&
      live.report.buildOutputState.length > 0 &&
      live.report.buildOutputManifest.realFileMutationPerformed === false,
    `${live.report.buildOutputState} id=${live.report.connectionId}`,
  );
  ...
  if (failed > 0) {
    console.log('CONNECTED_BUILD_EXECUTION_FOUNDATION_REQUIRES_FIXES');
    process.exit(1);
  }

  console.log(CONNECTED_BUILD_EXECUTION_FOUNDATION_PASS_TOKEN);
  console.log('');
  console.log('Build output states verified:');
  console.log(`  BUILD_OUTPUT_PROVEN:           ${proven.report.buildOutputState}`);
  console.log(`  BUILD_OUTPUT_PARTIALLY_PROVEN: ${partial.report.buildOutputState}`);
  console.log(`  BUILD_OUTPUT_BLOCKED:          ${blocked.report.buildOutputState}`);
  console.log(`  INSUFFICIENT_EVIDENCE:         ${insufficient.report.buildOutputState}`);
  console.log(`  Live repo:                     ${live.report.buildOutputState}`);
  console.log('');
  console.log('Report: architecture/CONNECTED_BUILD_EXECUTION_FOUNDATION_REPORT.md');
