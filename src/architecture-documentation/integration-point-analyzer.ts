/**
 * Architecture Documentation — integration point analyzer.
 */
...
import { getCachedIntegrationAnalysis, setCachedIntegrationAnalysis } from './architecture-documentation-cache.js';

export interface IntegrationPointSnapshot {
  registryIntegrationCount: number;
  uvlIntegrationCount: number;
  validationIntegrationCount: number;
...
export function analyzeIntegrationPoints(
  ...
  snapshot: IntegrationPointSnapshot,
): IntegrationPointAnalysis {
  const cacheKey = [
  ...
  const cached = getCachedIntegrationAnalysis(cacheKey);
  ...
  const checks: Array<[boolean | undefined, string, string]> = [
    ...
    [input.missingValidationIntegrationGuidance, 'missing_validation_integration_guidance', 'validation_integrations'],
  ...
  const systemBonus =
    ...
    + (snapshot.validationIntegrationCount > 0 ? 7 : 0);
  ...
  const result: IntegrationPointAnalysis = {
    integrationCoverageScore,
    undocumentedIntegrations: uniqueUndocumented,
    integrationWarnings,
  };

  setCachedIntegrationAnalysis(cacheKey, result);
  return result;
}
...
export function resetIntegrationPointAnalyzerForTests(): void {
  integrationAnalysisCount = 0;
}

export function listBaseIntegrations(): readonly string[] {
