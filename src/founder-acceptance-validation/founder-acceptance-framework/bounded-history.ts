/**
 * Founder Acceptance Framework — bounded evaluation history.
 */

import type { FounderAcceptanceRecord } from './founder-acceptance-types.js';
import { DEFAULT_MAX_FOUNDER_ACCEPTANCE_HISTORY_SIZE } from './founder-acceptance-types.js';

interface HistoryEntry {
  recordId: string;
  frameworkCompleteness: FounderAcceptanceRecord['frameworkCompleteness'];
  dimensionCount: number;
  recordedAt: number;
}

const history: HistoryEntry[] = [];
let maxHistorySize = DEFAULT_MAX_FOUNDER_ACCEPTANCE_HISTORY_SIZE;

export function recordFounderAcceptanceHistory(record: FounderAcceptanceRecord): void {
  history.push({
    recordId: record.recordId,
    frameworkCompleteness: record.frameworkCompleteness,
    dimensionCount: record.dimensionCount,
    recordedAt: Date.now(),
  });
  while (history.length > maxHistorySize) {
    history.shift();
  }
}

export function getFounderAcceptanceHistory(): readonly HistoryEntry[] {
  return [...history];
}

export function getFounderAcceptanceHistorySize(): number {
  return history.length;
}

export function clearFounderAcceptanceHistory(): void {
  history.length = 0;
}

export function resetFounderAcceptanceHistoryForTests(): void {
  history.length = 0;
  maxHistorySize = DEFAULT_MAX_FOUNDER_ACCEPTANCE_HISTORY_SIZE;
}
