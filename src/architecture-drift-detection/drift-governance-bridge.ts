/**
 * Drift governance bridge — Phase 6 + 7 + 8 + 9 stack.
 * Observer only. No governance bypass.
 */
...
export function assertNoRegistryRuntimeMutation(): boolean {
  const registry = getDevPulseV2Owner('architecture_drift_detection');
  return registry.ownerModule === ARCHITECTURE_DRIFT_DETECTION_OWNER_MODULE && registry.phase === 9.4;
}

export function assertDistinctFromProtectedModules(): boolean {
  const drift = getDevPulseV2Owner('architecture_drift_detection');
  const protectedOwners = [
    getDevPulseV2Owner('world2_learning_loop'),
    ...
    getDevPulseV2Owner('controlled_execution_bridge'),
  ];
  return protectedOwners.every((o) => o.ownerModule !== drift.ownerModule);
}

export function assertNoDuplicateDriftDetection(): boolean {
...
export function assertObserverNotSourceOfTruth(): boolean {
  const drift = getDevPulseV2Owner('architecture_drift_detection');
  return drift.description.includes('observer') || drift.description.includes('detect');
}

export function getDriftGovernanceSummary(): string {
...
export function validateDriftGovernance(input: DriftAnalysisInput): GovernanceValidationResult {
  ...
  gates.push({ gateId: 'gov-w2-0001', gateType: 'WORLD2_PROTECTION', status: 'OPEN', description: 'World 2 protected — drift observer only' });
  gates.push({ gateId: 'gov-w1-0001', gateType: 'WORLD1_PROTECTION', status: 'OPEN', description: 'World 1 protected — no modification via drift detection' });
  gates.push({ gateId: 'gov-obs-0001', gateType: 'OBSERVER_NOT_SOURCE_OF_TRUTH', status: 'OPEN', description: 'Drift detection is observer — does not become source of truth' });
