/**
 * Cross-device continuity governance bridge — Phase 8 + Phase 6 stack.
...
import type { ContinuityInput, GateRecord, GovernanceStatus } from './types.js';
import { CROSS_DEVICE_CONTINUITY_FOUNDATION_OWNER_MODULE, DEPENDENCY_SYSTEMS } from './types.js';
...
export function assertGovernanceDependenciesPresent(): boolean {
  return (
    ...
    getDevPulseV2Owner('mobile_approval_flow_foundation').ownerModule ===
      MOBILE_APPROVAL_FLOW_FOUNDATION_OWNER_MODULE &&
    getDevPulseV2Owner('world2_workspace_foundation').ownerModule.length > 0 &&
...
export function assertNoRegistryRuntimeMutation(): boolean {
  const registry = getDevPulseV2Owner('cross_device_continuity_foundation');
  return (
    registry.ownerModule === CROSS_DEVICE_CONTINUITY_FOUNDATION_OWNER_MODULE && registry.phase === 8.5
  );
}

export function assertDistinctFromMobileApprovalFlowFoundation(): boolean {
  ...
  return continuity.ownerModule !== approval.ownerModule;
}

export function getContinuityGovernanceSummary(): string {
...
export function validateContinuityGovernance(input: ContinuityInput): GovernanceValidationResult {
