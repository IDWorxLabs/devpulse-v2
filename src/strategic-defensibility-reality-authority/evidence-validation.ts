/**
 * Shared evidence validation helpers for defensibility analyzers.
 */

import { FABRICATED_EVIDENCE_SOURCES } from './strategic-defensibility-registry.js';
import type { ObservedEvidenceBase } from './strategic-defensibility-types.js';
...
export function blockedByUpstreamOnlySignals(input: {
  ...
  return Boolean(input.revenueOnly || input.adoptionOnly || input.marketExpansionOnly);
}

export function moatObserved(score: number, threshold = 65): boolean {
