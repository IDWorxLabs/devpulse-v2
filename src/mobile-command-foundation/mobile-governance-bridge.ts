/**
 * Mobile governance bridge — connects to Phase 6 and Phase 7 stack without bypassing governance.
...
import type { GateRecord, GovernanceStatus, MobileSessionInput } from './types.js';
import { DEPENDENCY_SYSTEMS, MOBILE_COMMAND_FOUNDATION_OWNER_MODULE } from './types.js';
...
export function assertNoRegistryRuntimeMutation(): boolean {
  const registry = getDevPulseV2Owner('mobile_command_foundation');
  return (
    registry.ownerModule === MOBILE_COMMAND_FOUNDATION_OWNER_MODULE && registry.phase === 8.1
  );
}

export function assertDistinctFromControlledExecutionBridge(): boolean {
  const mobile = getDevPulseV2Owner('mobile_command_foundation');
  const bridge = getDevPulseV2Owner('controlled_execution_bridge');
  return mobile.ownerModule !== bridge.ownerModule;
}

export function getMobileGovernanceSummary(): string {
...
export function validateGovernance(input: MobileSessionInput): GovernanceValidationResult {
  ...
  if (input.governanceStatus === 'PENDING') {
    gates.push({
      gateId: 'gov-status-0002',
      gateType: 'GOVERNANCE_STATUS',
      status: 'REQUIRED',
      description: 'governanceStatus is PENDING — approval-required capabilities blocked',
    });
    return { valid: false, reason: 'Governance pending', gates };
  }

  const stackPresent = assertGovernanceDependenciesPresent();
  gates.push({
    gateId: 'gov-stack-0001',
    gateType: 'GOVERNANCE_STACK',
    status: stackPresent ? 'OPEN' : 'CLOSED',
    description: stackPresent ? 'Phase 6/7 governance stack present' : 'Governance stack incomplete',
  });

  const noBypass = assertNoGovernanceBypass();
  gates.push({
    gateId: 'gov-bypass-0001',
    gateType: 'GOVERNANCE_BYPASS',
    status: noBypass ? 'OPEN' : 'CLOSED',
    description: noBypass ? 'No governance bypass detected' : 'Governance bypass attempt detected',
  });
