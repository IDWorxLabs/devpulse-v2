/**
 * AiDevEngine Capability Audit V3 — recommended roadmap (fresh from evidence).
 *
 * Does NOT inherit V2 priorities. Real Build Execution Pipeline is COMPLETE.
 */

import type { RoadmapPriority } from './capability-audit-types.js';

export const RECOMMENDED_ROADMAP_V3: readonly RoadmapPriority[] = [
  {
    rank: 1,
    phase: 'UVL Verification Execution',
    action: 'EXTEND',
    rationale:
      'Real Build Execution V1.1 proves build/preview/launch at 100% but verifiedCount is 0/15. UVL operational coverage ~6%. This is the highest-priority remaining gap.',
    impact: 'CRITICAL',
    dependencies: ['UVL Verification Hub V1', 'Real Build Execution Pipeline V1.1', 'Feature Reality Validation'],
  },
  {
    rank: 2,
    ...
    action: 'REGISTER',
    rationale:
      'Real Build Execution V1/V1.1, CQI Maturity V1, Capability Audit V2/V3, and Production Readiness Gate need constitutional registration.',
    impact: 'HIGH',
  ...
  },
  {
    ...
    action: 'EXTEND',
    rationale:
      'Real Build Execution proves 15 CRUD-adjacent categories at 100%; Code Generation Engine still limited to 5 profiles. Extend to unlock 58-category vision.',
    impact: 'HIGH',
  ...
  },
  {
    ...
    action: 'BUILD',
    rationale:
      'Launch readiness validates blueprint suites; production deployment, monitoring, and rollback remain unvalidated. Score: 33/100.',
    impact: 'CRITICAL',
  ...
  },
  {
    ...
    action: 'EXTEND',
    rationale:
      'World2 modules validate in isolation; dry-run bridge does not activate real execution. Connect to Real Build Execution after verification wiring.',
    impact: 'HIGH',
    dependencies: ['UVL Verification Execution', 'Real Build Execution Pipeline V1.1'],
  },
  {
  ...
  },
  {
    ...
    action: 'BUILD',
    rationale:
      'No validated cloud runtime or production deployment. Defer until UVL verification and production readiness gate are proven locally.',
    impact: 'HIGH',
...
export function buildRecommendedRoadmap(): {
  priorities: readonly RoadmapPriority[];
  world2IsNextPhase: boolean;
  nextPriority: string;
} {
  const activePriorities = RECOMMENDED_ROADMAP_V3.filter((p) => p.action !== 'COMPLETE');
  return {
    priorities: RECOMMENDED_ROADMAP_V3,
    world2IsNextPhase: false,
    nextPriority: activePriorities[0]?.phase ?? 'UVL Verification Execution',
  };
}
