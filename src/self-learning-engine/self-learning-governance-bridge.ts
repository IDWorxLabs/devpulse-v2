/**
 * Self-learning governance bridge — Phase 6 + 7 + 8 + 9 stack.
 * Recording only. No governance bypass.
 */
...
import { DEPENDENCY_SYSTEMS, DUPLICATE_PATTERNS, SELF_LEARNING_ENGINE_OWNER_MODULE } from './types.js';
...
export function assertNoRegistryRuntimeMutation(): boolean {
  const registry = getDevPulseV2Owner('self_learning_engine');
  return registry.ownerModule === SELF_LEARNING_ENGINE_OWNER_MODULE && registry.phase === 9.3;
}

export function assertDistinctFromWorld2LearningLoop(): boolean {
  const selfLearning = getDevPulseV2Owner('self_learning_engine');
  const world2Loop = getDevPulseV2Owner('world2_learning_loop');
  return selfLearning.ownerModule !== world2Loop.ownerModule;
}

export function assertNoDuplicateSelfLearningEngine(): boolean {
  ...
  return DUPLICATE_PATTERNS.every((pattern) => {
    ...
    const competing = [...registeredModules].filter(
      (m) => (m.includes(normalized) || m.includes(pattern.replace(/\s+/g, ''))) && m !== selfLearningOwner,
    );
    return competing.length === 0;
  });
}

export function getSelfLearningGovernanceSummary(): string {
...
export function validateSelfLearningGovernance(input: LearningEventInput): GovernanceValidationResult {
  const gates: GateRecord[] = [];

  if (input.governanceStatus === 'FAIL') {
    gates.push({
      gateId: 'gov-fail-0001',
  ...
  gates.push({
    ...
    status: 'OPEN',
    description: 'Phase 6 governance stack referenced — no bypass',
  });
  ...
  gates.push({
    ...
    status: 'OPEN',
    description: 'World 2 protected — learning records only, no mutation',
  });
  ...
  gates.push({
    ...
    status: 'OPEN',
    description: 'World 1 protected — no modification via learning',
  });
