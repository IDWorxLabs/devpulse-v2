...
export function buildReasoningVisibilityRecord(query: string): ReasoningVisibilityRecord {
  ...
  return {
    ...
    verificationRuntimeBasis: `UVL runtime advisory from ${evidence.length} evidence items and ${blockers.length} blockers — Phase 16.7 provider registration and session lifecycle only.`,
    registeredProviders: [
    ...
    verificationLifecycle: [
      ...
      'READY → session ready for future execution',
      'RUNNING → lifecycle transition only',
      'COMPLETED → session lifecycle complete',
    ],
    verificationWarnings: [
      'No verification execution',
      'No evidence generation or report orchestration',
      'No auto-fix or project scoring',
    ],
    verificationRegistryBasis: `Verification registry advisory from ${evidence.length} evidence items and ${blockers.length} blockers — Phase 16.8 registry metadata only, no execution or orchestration.`,
    verificationTargets: [
      'WORLD2_TARGET — World 2 Verification',
      'PREVIEW_TARGET — Preview Verification',
      'SELF_VISION_TARGET — Self Vision Verification',
      'UI_INSPECTION_TARGET — UI Inspection Verification',
      'INTERACTION_TARGET — Interaction Verification',
      'VISUAL_VERIFICATION_TARGET — Visual Verification',
      'RUNTIME_TARGET — Runtime Verification',
      'COMMAND_CENTER_TARGET — Command Center Verification',
      'PROJECT_VAULT_TARGET — Project Vault Verification',
      'OPERATOR_FEED_TARGET — Operator Feed Verification',
      'TRUST_TARGET — Trust Verification',
    ],
    verificationDependencies: [
      'PREVIEW_TARGET → SELF_VISION_TARGET',
      'SELF_VISION_TARGET → UI_INSPECTION_TARGET',
      'UI_INSPECTION_TARGET → INTERACTION_TARGET',
      'INTERACTION_TARGET → VISUAL_VERIFICATION_TARGET',
      'WORLD2_TARGET → RUNTIME_TARGET',
      'RUNTIME_TARGET → TRUST_TARGET',
    ],
    verificationRequirements: [
      'PREVIEW_TARGET — PREVIEW_SESSION_EVIDENCE required',
      'SELF_VISION_TARGET — OBSERVATION_SESSION_EVIDENCE required',
      'VISUAL_VERIFICATION_TARGET — VERIFICATION_EVIDENCE required',
      'TRUST_TARGET — TRUST_POLICY prerequisite',
    ],
    orchestrationBasis: `Verification orchestrator advisory from ${evidence.length} evidence items and ${blockers.length} blockers — Phase 16.9 execution planning only, no provider execution.`,
    executionOrder: [
      'WORLD2_TARGET → RUNTIME_TARGET → TRUST_TARGET',
      'PREVIEW_TARGET → SELF_VISION_TARGET → UI_INSPECTION_TARGET',
      'UI_INSPECTION_TARGET → INTERACTION_TARGET → VISUAL_VERIFICATION_TARGET',
      'PROJECT_VAULT_TARGET → COMMAND_CENTER_TARGET → OPERATOR_FEED_TARGET',
    ],
    parallelGroups: [
      'vpg-0001: WORLD2_TARGET, PREVIEW_TARGET, PROJECT_VAULT_TARGET',
      'vpg-0002: RUNTIME_TARGET, SELF_VISION_TARGET',
    ],
    blockedTargets: blockers.length > 0 ? ['BLOCKED_TARGET_EXAMPLE'] : [],
    waitingTargets: [
