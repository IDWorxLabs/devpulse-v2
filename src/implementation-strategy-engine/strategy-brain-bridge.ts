/**
 * Central Brain bridge — awareness owner unchanged; strategy engine publishes summaries only.
 */
...
import { summarizeStrategy } from './implementation-strategy-engine.js';
import type { ImplementationStrategy, StrategySummary } from './types.js';

let latestPublishedSummary: StrategySummary | null = null;

export function publishStrategySummary(strategy: ImplementationStrategy): StrategySummary {
...
export function getLatestStrategySummary(): StrategySummary | null {
  return latestPublishedSummary ? { ...latestPublishedSummary } : null;
}

export function getBrainSummariesForStrategyDetection(): string[] {
...
export function assertCentralBrainOwnershipUnchanged(): boolean {
  ...
  return (
    ...
    typeof (brain as { generateImplementationStrategy?: unknown }).generateImplementationStrategy ===
      'undefined'
  );
}
...
export function resetStrategyBrainBridgeForTests(): void {
