/**
 * AIDEVENGINE_MULTI_DOMAIN_BUILD_PROOF_V1 — domain-aware launch evidence handoff (V1.4 parity, no gate weakening).
 */
...
import type { DomainBehaviourEvidenceRecord } from './multi-domain-scenario-types.js';
import { getPrimaryEntity } from '../universal-feature-contract-intelligence/universal-feature-contract-builder.js';

export function applyMultiDomainLaunchEvidenceHandoff(input: {
  bundle: LaunchEvidenceBundle;
  domainBehaviour: DomainBehaviourEvidenceRecord;
  ...
  const { bundle, visualRuntime, productArchitectureEvidence, domainBehaviour } = input;
  ...
  const contractId = bundle.buildMaterialization.buildReadyContractId ?? 'multi-domain-build-proof-handoff';
  ...
  entries.push({
    ...
    consumed: true,
    fieldsUsed: ['requirementConfidenceScore', 'canProceedToPlanning', 'coverageMatrix', 'enrichedPrompt'],
  ...
  entries.push({
    ...
    fieldsIgnored: ['generic pattern without workspace scope'],
    fieldsUnsupported: [],
  ...
  const featureChecks: FeatureRealityCheck[] = domainBehaviour.behaviours
    .filter((b) => b.id !== 'browserBuildArtifactExists')
    .map((item) => ({
      id: `handoff-${item.id}`,
      category: item.category,
      label: `${item.label} (source-derived)`,
      featureId: item.id,
      passed: item.passed,
  ...
  for (const runtimeCheck of visualRuntime.checks.filter((c) => c.category === 'runtime-ui')) {
    featureChecks.push({
      id: runtimeCheck.id,
      category:
        runtimeCheck.id.includes('filter') ? 'discoverability' : runtimeCheck.id.includes('count') ? 'ux' : 'execution',
  ...
  entries.push({
    ...
    consumed: true,
    fieldsUsed: ['domain UVL behaviour evidence', 'bounded runtime UI checks when Playwright supported'],
    fieldsIgnored: [],
  ...
  const visualAssessment = registerSourceDerivedBlueprintVisualAssessment({
    ...
    viewportEvidence: visualRuntime.playwrightSupported
      ...
      : [`multi-domain-proof: ${visualRuntime.playwrightUnsupportedReason ?? 'viewport not measured'}`],
  });
  entries.push({
    readOnly: true,
    authority: 'Universal App Blueprint Visual',
    consumed: true,
    fieldsUsed: ['blueprint structure inspection', 'dist artifact path', 'bounded visual-runtime-evidence checks'],
    fieldsIgnored: [],
  ...
  const engineeringAssessment = registerSourceDerivedEngineeringRealityAssessment({
    ...
    buildAnalysis: {
      ...
      warnings: [],
      detail: bundle.buildMaterialization.npmBuildOk ? 'Multi-domain build proof handoff' : 'Build not proven',
    },
  });
  entries.push({
    readOnly: true,
  ...
  if (visualRuntime.playwrightSupported) {
    const contract = buildUniversalFeatureContract({
      ...
      profile: input.codegenProfile,
    });
    const primaryEntity = getPrimaryEntity(contract);
    ...
    const universalChecks: UniversalFeatureRealityCheck[] = visualRuntime.checks
      ...
      .map((c) => ({
        id: c.id,
        category:
          c.id.includes('filter') ? 'discoverability' : c.id.includes('count') ? 'workflow' : 'execution',
    ...
    entries.push({
      ...
      consumed: true,
      fieldsUsed: ['bounded runtime UI checks', 'contract from enriched prompt with profile override'],
  ...
  entries.push({
    ...
    fieldsUsed: [
      ...
      'UVL hub assessment before AFLA',
    ],
    fieldsIgnored: [],
    fieldsUnsupported: [],
  ...
  entries.push({
    ...
    consumed: true,
    fieldsUsed: ['buildProofHandoff', 'workspaceDir', 'visual-runtime-evidence', 'registered assessments'],
    fieldsIgnored: [],
    fieldsUnsupported: [],
  ...
  const founderEvidenceAfterHandoff = collectFounderLaunchEvidence({
    ...
    useRegisteredProductArchitecture: true,
    useRegisteredVerificationHub: true,
    useRegisteredRequirementDiscovery: input.useRegisteredRequirementDiscovery ?? false,
  });
  entries.push({
    readOnly: true,
    authority: 'Founder Launch Readiness (evidence collector)',
    consumed: true,
    fieldsUsed: ['workspaceDir', 'buildReality', 'registered assessments', 'visual-runtime-evidence'],
    fieldsIgnored: [],
    fieldsUnsupported: [],
