/**
 * Parallel Build Orchestration — dependency chain management.
 */

import type { OrchestrationProjectInput } from './orchestration-types.js';
import { MAX_DEPENDENCY_CHAIN_DEPTH } from './orchestration-types.js';
import { getCachedDependencies, setCachedDependencies } from './orchestration-cache.js';
...
export function buildDependencyChains(projects: OrchestrationProjectInput[]): DependencyBuildResult {
  ...
  function detectCycle(projectId: string, path: string[]): void {
    if (path.length > MAX_DEPENDENCY_CHAIN_DEPTH) {
  ...
  if (cycles.length === 0 && missing.length === 0) {
    setCachedDependencies(cacheKey, uniqueChains);
  }
...
function buildProjectChain(
  ...
  if (visited.has(projectId) || visited.size > MAX_DEPENDENCY_CHAIN_DEPTH) {
    return [projectId];
  }
...
export function getDependencyCount(projects: OrchestrationProjectInput[]): number {
